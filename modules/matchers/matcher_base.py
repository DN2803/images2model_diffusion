...
from modules.extractors.extractor_base import ExtractorBase

from utils import Quality, TileSelection, Timer, get_size_by_quality, logger
from models.extractors.superpoint import SuperPoint
from utils.io.h5 import get_features, get_matches
...
from utils.visualization import viz_matches_cv2, viz_matches_mpl
...
class MatcherBase(metaclass=ABCMeta):
    ...
    def __init__(self, custom_config) -> None:
        ...
        if self._config["general"]["tile_selection"] != TileSelection.NONE:
            ...
            model_params = set(map(lambda n: n[0], self._preselction_matcher.named_parameters()))
            diff = model_params - dict_params
            if len(diff) > 0:
                state_dict = {k.replace('matcher.', 'matcher.net.'): v for k, v in state_dict.items()}
            self._preselction_matcher.load_state_dict(state_dict, strict=False)
        else:
